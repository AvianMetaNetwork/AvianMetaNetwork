...
clements_mismatches <- read_csv(here::here("./data/L1/aux_mismatches_with_clements_best_match.csv"))
```
...
See documentation in `X_taxonomy_functions.R` script for details
...
Use fuzzy logic function ( see X_taxonomy_functions.R ) to find closest match from the CHECKLIST reference list
...
unique_taxa_list <- unique(c(int.raw$species1_scientific, int.raw$species2_scientific))
...
int.raw$taxa1_clements <- ifelse(
  int.raw$species1_scientific %in% mismatch, 
  NA, 
  int.raw$species1_scientific
)

int.raw$taxa2_clements <- ifelse(
  int.raw$species2_scientific %in% mismatch, 
  NA, 
...
int.raw <- int.raw %>%
  left_join(lookup, by = c("species1_scientific" = "old_name")) %>%
  rename(new1 = new_name) %>%
  left_join(lookup, by = c("species2_scientific" = "old_name")) %>%
...
remaining_mismatch <- unique(c(int.raw$species1_scientific[is.na(int.raw$taxa1_clements)], 
                               int.raw$species2_scientific[is.na(int.raw$taxa2_clements)]))
...
if(length(remaining_mismatch) > 0){
  write.csv(remainin_mismatch, here::here("./data/L1/aux_mismatches_with_clements_TOFIXMANUALLY.csv"))
