...
source(here::here('R/L1/X_taxonomy_functions.R'))
...
```{r}
cac_spp1 <- intxns.db.subset[intxns.db.subset$species1_scientific %in% unique(splist$scientific_name_clements2024),]
nocac_spp1 <- intxns.db.subset[!(intxns.db.subset$species1_scientific %in% unique(splist$scientific_name_clements2024)),]

nocac_spp1_flipped <- nocac_spp1 %>% 
  rename(
    species2_common = species1_common, 
    species1_common = species2_common, 
    species2_scientific = species1_scientific, 
    species1_scientific = species2_scientific, 
    effect_sp1_on_sp2 = effect_sp2_on_sp1, 
    effect_sp2_on_sp1 = effect_sp1_on_sp2
  ) %>% 
  relocate(colnames(nocac_spp1))

# Check to make sure all spp1 are now CAC species (value should be 0) 
length(nocac_spp1_flipped$species1_scientific[!(nocac_spp1_flipped$species1_scientific %in% unique(splist$scientific_name_clements2024))])

# Recombine 
intxns.db.subset <- rbind(cac_spp1, nocac_spp1_flipped)

# Check to make sure network contains all CAC species 
splist$scientific_name_clements2024.combo[!(unique(splist$scientific_name_clements2024.combo) %in% c(intxns.db.subset$species1_scientific, intxns.db.subset$species2_scientific))]

splist$scientific_name_clements2024[!(unique(splist$scientific_name_clements2024) %in% c(intxns.db.subset$species1_scientific, intxns.db.subset$species2_scientific))]
