...
network_spp <- unique(c(intxns.db.subset.new$species1_scientific, intxns.db.subset.new$species2_scientific))
...
# List of characters to re-assign
old_facil_sub<- c("facilitation-feeding")
new_facil_sub <- "facilitation-foraging"
# Re-assign values 
...
Change species name column so that it reflects the broad range of taxonomic resolutions of the data
```{r}
intxns.db.subset.new <- intxns.db.subset.new %>% 
  rename(
